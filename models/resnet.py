...
class ResNet(AbstractResNet):
    ...
    def blockone_feature(self, x):
        out = self.maxpool(F.relu(self.bn1(self.conv1(x))))
        out = self.layer1(out)
        out = torch.nn.AvgPool2d(56, stride=1)(out)
        out = out.view(out.size(0), -1)
        return out

    def blocktwo_feature(self, x):
        out = self.maxpool(F.relu(self.bn1(self.conv1(x))))
        out = self.layer1(out)
        out = self.layer2(out)
        out = torch.nn.AvgPool2d(28, stride=1)(out)
        out = out.view(out.size(0), -1)
        return out

    def blockthree_feature(self, x):
        out = self.maxpool(F.relu(self.bn1(self.conv1(x))))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = torch.nn.AvgPool2d(14, stride=1)(out)
        out = out.view(out.size(0), -1)
        return out
...
class ResNetCifar(AbstractResNet):
    ...
    def forward_features(self, x, fc_params=None):
        feat = self.features(x)
        feat = self.avgpool(feat)
        feat = feat.view(feat.size(0), -1)
        return feat
