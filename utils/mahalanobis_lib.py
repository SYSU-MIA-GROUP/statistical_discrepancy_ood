...
def model_feature_list(model, x, model_arch):
    out_list = []
    y = None
    if model_arch in ["resnet101", "resnet18", "resnet50"]:
        y, out_list = model.feature_list(x)
    return y, out_list


def compute_M_score(model, inputs, pack, noise, num_classes):
    '''
    Compute the proposed Mahalanobis confidence score on inputs dataset
    ...
    out_features = model.forward_features(data)
    out_features = out_features.view(out_features.size(0), out_features.size(1), -1)
    ...
    with torch.no_grad():
        noise_out_features = model.forward_features(Variable(tempInputs))
        noise_out_features = noise_out_features.view(noise_out_features.size(0), noise_out_features.size(1), -1)
        noise_out_features = torch.mean(noise_out_features, 2)
        noise_gaussian_score = 0
        for i in range(num_classes):
...
def get_M_score(model, input, args, num_classes):
    print("Using noise", args.noise)
    pack = (args.method_args['sample_mean'], args.method_args['precision'])
    scores = compute_M_score(model, input, pack, args.noise, num_classes)
    return scores
